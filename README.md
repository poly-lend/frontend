...
Frontend applications for the PolyLend peer-to-peer lending protocol on Polymarket positions.

## Structure

```
web/        # Main web application (borrower & lender dashboards)
...
Contract addresses and constants are in `web/src/config.ts`:
...
Static landing page built with React + Vite.
...
Both apps are containerized via Dockerfiles and served as static files through Caddy. See the `infra/` repo for docker-compose configuration.
